	...
	{Name: "Base", Desc: "these are the best params", Sheets: params.Sheets{
		"Network": &params.Sheet{
			...
			{Sel: ".Back", Desc: "top-down back-projections MUST have lower relative weight scale, otherwise network hallucinates",
				...
				{Sel: "#Race", Desc: "output definitely needs lower inhib -- true for smaller layers in general",
				Params: params.Params{
					"Layer.Inhib.Layer.Gi": "1.4",
				}},
			{Sel: "#Age", Desc: "output definitely needs lower inhib -- true for smaller layers in general",
				Params: params.Params{
					"Layer.Inhib.Layer.Gi": "1.4",
				}},
				{Sel: "#Gender", Desc: "output definitely needs lower inhib -- true for smaller layers in general",
				Params: params.Params{
					"Layer.Inhib.Layer.Gi": "1.4",
				}},
				{Sel: "#PosEval", Desc: "Typical inhibition for ScalarValue Layer",
				Params: params.Params{
					"Layer.Inhib.Layer.Gi": "1.8",
				}},
				{Sel: "#NegEval", Desc: "Typical inhibition for ScalarValue Layer",
				Params: params.Params{
					"Layer.Inhib.Layer.Gi": "1.8",
				}},
...
type Sim struct {
	...
	PosEvalTsr 	 *etensor.Float32 			 `view:"-" desc:"for holding Positive Eval layer values"`
    NegEvalTsr 	 *etensor.Float32 			 `view:"-" desc:"for holding Negative Eval layer values"`
	TmpVals		 []float32					 `view:"-" desc:"for holding temporary values for Scalar value decoding"`
...
func (ss *Sim) New() {
	...
	ss.TestInterval = 5
	ss.LayStatNms = []string{"Conjunction", "Profession", "PosEval", "NegEval"}
}
...
func (ss *Sim) ConfigEvalTsrs() {

      if ss.PosEvalTsr == nil {

         ss.PosEvalTsr = etensor.NewFloat32([]int{1, 11}, nil, nil)

}

      if ss.NegEvalTsr == nil {

         ss.NegEvalTsr = etensor.NewFloat32([]int{1, 11}, nil, nil)
...
func (ss *Sim) ConfigNet(net *leabra.Network) {
	net.InitName(net, "IR")
	inp := net.AddLayer2D("Input", 1, 17, emer.Input)
	hid := net.AddLayer2D("Hidden", 15, 15, emer.Hidden)
	gender := net.AddLayer2D("Gender", 1, 2, emer.Target)
	race := net.AddLayer2D("Race", 1, 2, emer.Target)
	conj := net.AddLayer2D("Conjunction", 1, 8, emer.Target)
	pos := net.AddLayer2D("PosEval", 1, 11, emer.Target)
	neg := net.AddLayer2D("NegEval", 1, 11, emer.Target)
	cont := net.AddLayer2D("Context", 1, 6, emer.Target)
	hid2 := net.AddLayer2D("Hidden2", 10, 10, emer.Hidden)
	attr := net.AddLayer2D("Attributes", 1, 8, emer.Target)
	prof := net.AddLayer2D("Profession", 1, 6, emer.Target)
	
	
	
	full := prjn.NewFull()
	net.ConnectLayers(inp, hid, full, emer.Forward)
	net.ConnectLayers(inp, gender, full, emer.Forward)
	net.ConnectLayers(inp, race, full, emer.Forward)
	net.BidirConnectLayers(gender, conj, full)
	net.BidirConnectLayers(race, conj, full)
	net.BidirConnectLayers(conj, pos, full)
	net.BidirConnectLayers(conj, neg, full)
	net.BidirConnectLayers(hid, cont, full)
	net.BidirConnectLayers(hid, attr, full)
	net.BidirConnectLayers(hid, prof, full)
	net.BidirConnectLayers(cont, hid2, full)
	net.BidirConnectLayers(cont, attr, full)
	net.BidirConnectLayers(hid2, prof, full)
	net.BidirConnectLayers(prof, pos, full)
	net.BidirConnectLayers(prof, neg, full)
	net.BidirConnectLayers(prof, attr, full)
	net.BidirConnectLayers(pos, attr, full)
	net.BidirConnectLayers(neg, attr, full)
	
	

	hid.SetRelPos(relpos.Rel{Rel: relpos.Above, Other: "Input", YAlign: relpos.Front, XAlign: relpos.Left, YOffset: 0})
	cont.SetRelPos(relpos.Rel{Rel: relpos.Above, Other: "Hidden", YAlign: relpos.Front, XAlign: relpos.Left, XOffset: 1})
	hid2.SetRelPos(relpos.Rel{Rel: relpos.Above, Other: "Context", YAlign: relpos.Front, XAlign: relpos.Left, XOffset: 1})
	prof.SetRelPos(relpos.Rel{Rel: relpos.Above, Other: "Hidden2", YAlign: relpos.Front, XAlign: relpos.Left, XOffset: 1})
	
	pos.SetRelPos(relpos.Rel{Rel: relpos.RightOf, Other: "Context", YAlign: relpos.Front, XAlign: relpos.Right, XOffset: 1})
	neg.SetRelPos(relpos.Rel{Rel: relpos.RightOf, Other: "PosEval", YAlign: relpos.Front, XAlign: relpos.Right, XOffset: 1})
	attr.SetRelPos(relpos.Rel{Rel: relpos.RightOf, Other: "Profession", YAlign: relpos.Front, XAlign: relpos.Right, XOffset: 8})
	
	gender.SetRelPos(relpos.Rel{Rel: relpos.RightOf, Other: "Input", YAlign: relpos.Front,  XAlign: relpos.Middle, XOffset: 1})
	race.SetRelPos(relpos.Rel{Rel: relpos.RightOf, Other: "Gender", YAlign: relpos.Front,  XAlign: relpos.Right, XOffset: 1})
	
	conj.SetRelPos(relpos.Rel{Rel: relpos.RightOf, Other: "Hidden", YAlign: relpos.Front, XAlign: relpos.Middle, XOffset: 1})
...
func (ss *Sim) ApplyInputs(en env.Env) {
	...
	// going to the same layers, but good practice and cheap anyway	
	 ...
	 lays := []string{"Input", "Race", "Gender", "Conjunction", "Context", "PosEval", "NegEval", "Profession", "Attributes"}   
     ...
     if ly.Nm == "PosEval" {
           ...
           pc.Encode(&ss.PosEvalTsr.Values, v, 11, false)
//           fmt.Printf("ss.PosEvalTsr is %v",ss.PosEvalTsr) //gives me 11 numbers (gaussian bump)
           ly.ApplyExt(ss.PosEvalTsr)

}

     if ly.Nm == "NegEval" {
           v := float32(pats.FloatVal1D(0))
           pc.Encode(&ss.NegEvalTsr.Values, v, 11, false)
 //          fmt.Printf("ss.NegEvalTsr is %v",ss.NegEvalTsr) //gives me 11 numbers (gaussian bump)
           ly.ApplyExt(ss.NegEvalTsr)
...
func (ss *Sim) TrialStats(accum bool) {
	out := ss.Net.LayerByName("Profession").(leabra.LeabraLayer).AsLeabra()
...
func (ss *Sim) ConfigPats() {
	...
	sch := etable.Schema{
		...
		{"Input", etensor.FLOAT32, []int{1, 17}, []string{"Y", "X"}},
		{"Context", etensor.FLOAT32, []int{1,8}, []string{"Y", "X"}},
		{"Profession", etensor.FLOAT32, []int{1, 4}, []string{"Y", "X"}},
		{"Conjunction", etensor.FLOAT32, []int{1, 8}, []string{"Y", "X"}},
		{"Race", etensor.FLOAT32, []int{1, 2}, []string{"Y", "X"}},
		{"Gender", etensor.FLOAT32, []int{1, 2}, []string{"Y", "X"}},
		{"Attributes", etensor.FLOAT32, []int{1, 8}, []string{"Y", "X"}},
		{"NegEval", etensor.FLOAT32, []int{1, 11}, []string{"Y", "X"}},
		{"PosEval", etensor.FLOAT32, []int{1, 11}, []string{"Y", "X"}},

	}
	dt.SetFromSchema(sch, 86)
	...
	patgen.PermutedBinaryRows(dt.Cols[8], 1, 1, 0)
	patgen.PermutedBinaryRows(dt.Cols[9], 1, 1, 0)

		
	dt.SaveCSV("IRTrain.tsv", etable.Tab, etable.Headers)
}
*/
...
func (ss *Sim) OpenPats() {
	...
	dt.SetMetaData("name", "TrainPats")
	dt.SetMetaData("desc", "Training patterns")
	dt.OpenCSV("IRTrain.tsv", etable.Tab)
	
	
	dt2 := ss.TestPats
	dt2.SetMetaData("name", "TestPats")
	dt2.SetMetaData("desc", "Testing patterns")
	dt2.OpenCSV("IRTest.tsv", etable.Tab)
...
func (ss *Sim) ConfigTrnEpcPlot(plt *eplot.Plot2D, dt *etable.Table) *eplot.Plot2D {
	plt.Params.Title = "IR Epoch Plot"
...
func (ss *Sim) LogTstTrl(dt *etable.Table) {
	...
	out1 := ss.Net.LayerByName("Profession").(leabra.LeabraLayer).AsLeabra()
	out2 := ss.Net.LayerByName("PosEval").(leabra.LeabraLayer).AsLeabra()
	out3 := ss.Net.LayerByName("NegEval").(leabra.LeabraLayer).AsLeabra()
	...
	ov1 := ss.ValsTsr("Profession")
	//ov2 := ss.ValsTsr("PosEval")
	// ov3 := ss.ValsTsr("NegEval")
	
	inp.UnitValsTensor(ivt, "Act")
	dt.SetCellTensor("InAct", row, ivt)
	
	out1.UnitValsTensor(ov1, "ActM")
	dt.SetCellTensor("ProfessionActM", row, ov1)
	
	// the following adopts the Decode go code from the popcode package, since the function in the popcode package seems to only return 0, no matter what.  
	out2.UnitVals(&ss.TmpVals, "ActM")
	// ov2 := pc.Decode(ss.TmpVals)
	n := 11
	min := float32(-0.5)
	max := float32(1.5)
	thr := float32(0.1)
	...
	sum = mat32.Max(sum, minsum)
	avg /= sum
	ov2 := avg
	
	
//	fmt.Printf("ov2 is %f", ov2) //
//	fmt.Printf("ss.TmpVals is %v",ss.TmpVals) //gives me 11 numbers (gaussian bump)

	dt.SetCellFloat("PosEvalActM", row, float64(ov2))
	
	
	out3.UnitVals(&ss.TmpVals, "ActM")
	// ov3 := pc.Decode(ss.TmpVals)
...
//  fmt.Printf("ov3 is %f", pc.Decode(ss.TmpVals))
//	fmt.Printf("ov3 is %f", ov3) //
	dt.SetCellFloat("NegEvalActM", row, float64(ov3))
...
func (ss *Sim) ConfigTstTrlLog(dt *etable.Table) {
	...
	sch = append(sch, etable.Schema{
		...
		{"ProfessionActM", etensor.FLOAT64, out1.Shp.Shp, nil},
		{"PosEvalActM", etensor.FLOAT64, nil, nil},
		{"NegEvalActM", etensor.FLOAT64, nil, nil},
	}...)
	dt.SetFromSchema(sch, nt)
}

func (ss *Sim) ConfigTstTrlPlot(plt *eplot.Plot2D, dt *etable.Table) *eplot.Plot2D {
	plt.Params.Title = "IR Test Trial Plot"
	...
	plt.SetColParams("ProfessionActM", eplot.Off, eplot.FixMin, 0, eplot.FixMax, 1)
	plt.SetColParams("PosEvalActM", eplot.Off, eplot.FixMin, 0, eplot.FixMax, 1)
	plt.SetColParams("NegEvalActM", eplot.Off, eplot.FixMin, 0, eplot.FixMax, 1)
	return plt
}
...
func (ss *Sim) LogTstEpc(dt *etable.Table) {
	...
	split.Agg(allsp, "ProfessionActM", agg.AggMean)
	split.Agg(allsp, "PosEvalActM", agg.AggMean)
	split.Agg(allsp, "NegEvalActM", agg.AggMean)
...
func (ss *Sim) ConfigTstEpcPlot(plt *eplot.Plot2D, dt *etable.Table) *eplot.Plot2D {
	plt.Params.Title = "IR Testing Epoch Plot"
	plt.Params.XAxisCol = "Epoch"
	plt.SetTable(dt)
	// order of params: on, fixMin, min, fixMax, max
	plt.SetColParams("Run", eplot.Off, eplot.FixMin, 0, eplot.FloatMax, 0)
...
// How could we increase it to the number of rows that is equal to number of test trials * number of cycles?
...
func (ss *Sim) LogTstCyc(dt *etable.Table, cyc int) {
	...
	if dt.Rows <= cyc {
 	...
 	dt.SetCellString("TrialName", cyc, ss.TestEnv.TrialName.Cur)
...
//	fmt.Printf("ss.TmpVals is %v",ss.TmpVals) //gives me 11 numbers (gaussian bump)
	...
	dt.SetCellFloat("PosEvalAct", cyc, float64(ov2))
	
	
	out3.UnitVals(&ss.TmpVals, "Act")
	// ov3 := pc.Decode(ss.TmpVals)
...
//  fmt.Printf("ov3 is %f", pc.Decode(ss.TmpVals))
//	fmt.Printf("ov3 is %f", ov3) //
	dt.SetCellFloat("NegEvalAct", cyc, float64(ov3))
...
//	}	
	sch = append(sch, etable.Schema{
		...
		{"PosEvalAct", etensor.FLOAT64, nil, nil},
		{"NegEvalAct", etensor.FLOAT64, nil, nil},
	}...)
...
func (ss *Sim) ConfigTstCycPlot(plt *eplot.Plot2D, dt *etable.Table) *eplot.Plot2D {
	plt.Params.Title = "IR Test Cycle Plot"
	plt.Params.XAxisCol = "Cycle"
	plt.SetTable(dt)
	// order of params: on, fixMin, min, fixMax, max
	plt.SetColParams("Cycle", eplot.Off, eplot.FixMin, 0, eplot.FloatMax, 0)
	plt.SetColParams("PosEvalAct", eplot.Off, eplot.FixMin, 0, eplot.FixMax, 1)
	plt.SetColParams("NegEvalAct", eplot.Off, eplot.FixMin, 0, eplot.FixMax, 1)
// for _, lnm := range ss.LayStatNms {
...
func (ss *Sim) ConfigNetView(nv *netview.NetView) {
	...
	nv.Scene().Camera.LookAt(mat32.Vec3{0.1, 0.1, 0}, mat32.Vec3{0, 1, 0})
		...
		labs := []string{"LHair MHair SHair FaceH !Face Rnd Sq Br !Br BlkH BrH Bln White Fld !Fld SmSkn WSkn White Black Brown",
		"White Black Asian Hisp", " M    F ", " Teen  Mid  Old" }
	nv.ConfigLabels(labs)

	lays := []string{"Input", "Race", "Gender",  "Age"}
	...
	for li, lnm := range lays {
		...
		lbl.Pose.Pos.Y += .1
		lbl.Pose.Pos.Z += .02
		lbl.Pose.Scale.SetMul(mat32.Vec3{.5, .2, 0.5})
	}
}
...
func (ss *Sim) ConfigGui() *gi.Window {
	...
	gi.SetAppName("IR")
	gi.SetAppAbout(`This demonstrates the Iterative Reprocessing model. See <a href="https://github.com/emer/emergent">emergent on GitHub</a>.</p>`)

	win := gi.NewMainWindow("IR", "Iterative Reprocessing Model", width, height)
	ss.Win = win
