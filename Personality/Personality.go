...
func (ss *Sim) Config() {
	//ss.ConfigPats() Only call first time you run the program (uncomment) to get properly 
	// configured datatable.  Afterwards comment out.
	ss.OpenPats()
	ss.ConfigEnv()
	ss.ConfigNet(ss.Net)
...
func (ss *Sim) OpenPats() {
	...
	dt.SetMetaData("name", "TrainPats")
	dt.SetMetaData("desc", "Training patterns")
	err := dt.OpenCSV("PersonalityTraining_Original.tsv", etable.Tab)
	if err != nil {
		log.Println(err)
	}
}
