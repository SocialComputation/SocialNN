...
/*
This is based on Figure 2 of Freeman and Ambady's 2010 Psychological Review paper, which examines the impact of task demands on
identification of the ethnicity and gender of different individuals
*/
package main
	...
	{Name: "Base", Desc: "these are the best params", Sheets: params.Sheets{
		"Network": &params.Sheet{
			...
			{Sel: "Layer", Desc: "generic params for all layers: lower gain, slower, soft clamp",
				...
				{Sel: "#Gender", Desc: "generic params for all layers: lower gain, slower, soft clamp",
				Params: params.Params{
					"Layer.Inhib.Layer.Gi": 	"1.2",		
				}},
				{Sel: "#Race", Desc: "generic params for all layers: lower gain, slower, soft clamp",
				Params: params.Params{
					"Layer.Inhib.Layer.Gi": 	"1.4",		
				}},
				{Sel: "#Docile", Desc: "generic params for all layers: lower gain, slower, soft clamp",
				Params: params.Params{
					"Layer.Inhib.Layer.Gi": 	"1.0",		
				}},
				{Sel: "#Aggressive", Desc: "generic params for all layers: lower gain, slower, soft clamp",
				Params: params.Params{
					"Layer.Inhib.Layer.Gi": 	"1.0",		
				}},
...
func (ss *Sim) New() {
	...
	ss.TstRecLays = []string{"GenderCues", "RaceCues", "Gender", "Race", "Aggressive", "Docile", "SexTask", "RaceTask"}
	ss.Defaults()
}
...
func (ss *Sim) ConfigNet(net *leabra.Network) {
	net.InitName(net, "PersonPerception")
	gcue := net.AddLayer2D("GenderCues", 1, 2, emer.Input)
	rcue := net.AddLayer2D("RaceCues", 1, 3, emer.Input)
	gender := net.AddLayer2D("Gender", 1, 2, emer.Input)
	race := net.AddLayer2D("Race", 1, 3, emer.Input)
	aggr := net.AddLayer2D("Aggressive", 1, 1, emer.Input)
	doc := net.AddLayer2D("Docile", 1, 1, emer.Input)
	stask := net.AddLayer2D("SexTask", 1, 1, emer.Input)
	rtask := net.AddLayer2D("RaceTask", 1, 1, emer.Input)
	...
	net.BidirConnectLayers(gcue, gender, full)
	net.BidirConnectLayers(rcue, race, full)
	net.BidirConnectLayers(gender, aggr, full)
	net.BidirConnectLayers(gender, doc, full)
	net.BidirConnectLayers(race, aggr, full)
	net.BidirConnectLayers(race, doc, full)
	net.ConnectLayers(aggr, doc, full, emer.Inhib)
	net.ConnectLayers(doc, aggr, full, emer.Inhib)
	net.ConnectLayers(stask, gender, full, emer.Forward)
	net.ConnectLayers(rtask, race, full, emer.Forward)
	
	
	
	gcue.SetRelPos(relpos.Rel{Rel: relpos.NoRel, YAlign: relpos.Front, XAlign: relpos.Middle})
	rcue.SetRelPos(relpos.Rel{Rel: relpos.RightOf, Other: "GenderCues", YAlign: relpos.Front, XAlign: relpos.Middle})
	
	gender.SetRelPos(relpos.Rel{Rel: relpos.Above, Other: "GenderCues",  YAlign: relpos.Front, XAlign: relpos.Middle})
	race.SetRelPos(relpos.Rel{Rel: relpos.Above, Other: "RaceCues", YAlign: relpos.Front, XAlign: relpos.Middle})
	
	doc.SetRelPos(relpos.Rel{Rel: relpos.Above, Other: "Gender", YAlign: relpos.Front, XAlign: relpos.Middle})
	aggr.SetRelPos(relpos.Rel{Rel: relpos.Above, Other: "Race", YAlign: relpos.Front, XAlign: relpos.Middle})
	
	stask.SetRelPos(relpos.Rel{Rel: relpos.Above, Other: "Docile", Space: 10, YAlign: relpos.Front, XAlign: relpos.Middle})
	rtask.SetRelPos(relpos.Rel{Rel: relpos.Above, Other: "Aggressive", Space: 10, YAlign: relpos.Front, XAlign: relpos.Middle})
...
func (ss *Sim) ApplyInputs(en env.Env) {
	...
	lays := []string{"GenderCues", "RaceCues", "SexTask", "RaceTask"}
...
// This can be used to create blank pattern files with the correct cell structure and Headers for your network
/*
func (ss *Sim) ConfigPats() {
	...
	dt.SetFromSchema(etable.Schema{
		...
		{"GenderCues", etensor.FLOAT32, []int{1, 2}, []string{"Y", "X"}},
		{"RaceCues", etensor.FLOAT32, []int{1, 3}, []string{"Y", "X"}},
		{"SexTask", etensor.FLOAT32, []int{1, 1}, []string{"Y", "X"}},
		{"RaceTask", etensor.FLOAT32, []int{1, 1}, []string{"Y", "X"}},
	}, 25)

	patgen.PermutedBinaryRows(dt.Cols[1], 1, 1, 0)
	patgen.PermutedBinaryRows(dt.Cols[2], 1, 1, 0)
	dt.SaveCSV("TestData.tsv", etable.Tab, etable.Headers)
}
*/
...
func (ss *Sim) OpenPats() {
	...
	ss.OpenPatAsset(ss.Pats, "TestingData.tsv", "PersonPerceptionPats", "Testing patterns")
...
func (ss *Sim) OpenPats() {
	...
	err := dt.OpenCSV("TestData.tsv", etable.Tab)
	if err != nil {
		log.Println(err)
	}
}
...
func (ss *Sim) ConfigTstCycPlot(plt *eplot.Plot2D, dt *etable.Table) *eplot.Plot2D {
	plt.Params.Title = "PersonPerception Test Cycle Plot"
...
func (ss *Sim) ConfigNetView(nv *netview.NetView) {
	...
	labs := []string{" Male Female", " Black White Asian", "Male Female", "Black White Asian", " ", " ", " ", " "}
	nv.ConfigLabels(labs)

	lays := []string{"GenderCues", "RaceCues", "Gender", "Race", "Aggressive", "Docile", "SexTask", "RaceTask"}
	...
	for li, lnm := range lays {
		...
		lbl.Pose.Pos.Y += .1
		lbl.Pose.Pos.Z += .005
		lbl.Pose.Scale.SetMul(mat32.Vec3{0.3, 0.06, 0.3})
	}
}
